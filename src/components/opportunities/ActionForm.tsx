...
export function ActionForm({ opportunityId, open, onClose, companyId }: Props) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [contacts, setContacts] = useState<ContactDTO[]>([]);
  const [channelTypes, setChannelTypes] = useState<ChannelTypeDTO[]>([]);
  const [form, setForm] = useState({
    type: "INTERVIEW" as OpportunityActionType,
  ...
  useEffect(() => {
    if (open) {
      // Charger les contacts de l'entreprise si disponible
      if (companyId) {
        fetchContacts(companyId).then(setContacts);
      } else {
        setContacts([]);
      }
      // Charger tous les types de canaux
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!opportunityId) {
      pushToast({ type: "error", title: "Erreur", description: "ID d'opportunité manquant" });
      return;
    }
    setLoading(true);
    try {
  ...
  return (
    <Modal open={open} title="Ajouter une action" onClose={onClose}>
